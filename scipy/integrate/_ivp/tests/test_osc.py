...
from scipy.integrate._ivp._osc import solve_ivp_osc
from scipy.integrate._ivp.ivp import OdeResult
from scipy.integrate._ivp import _riccati as _ric
...
class TestSolveIvpOsc:
    ...
    def test_validation_rtol_not_positive(self):
        """Test that rtol must be positive."""
    ...
    def test_dense_output_available(self):
        ...
        values = result.sol(np.array([0.0, 0.5, 1.0]))
...
class TestRiccatiSolverAccuracy:
    ...
    def test_dense_output_cosine(self):
        """Test dense output against harmonic oscillator solution."""
        omega = lambda x: 1.0 if np.isscalar(x) else np.ones_like(x)
        gamma = lambda x: 0.0 if np.isscalar(x) else np.zeros_like(x)
        ...
        y_sample = result.sol(t_sample)
        assert_allclose(y_sample[0], np.cos(t_sample), atol=1e-2)
...
class TestRiccatiPortedCases:
    ...
    def test_schrodinger_nondense_fwd_path_optimize(self, current_energy, target_diff):
        ...
        left_res = solve_ivp_osc(
            ...
            rtol=eps, epsilon_h=epsh, init_stepsize=init_step, events=None,
            vectorized=False, **solver_kwargs
        )
    ...
    def test_schrodinger_nondense_fwd_full_optimize(self):
        ...
        def energy_difference(current_energy):
            ...
            init_step = _ric.choose_osc_stepsize(
                omega_fun, gamma_fun, left_boundary, midpoint - left_boundary, epsh,
                **solver_params
            )
            left_res = solve_ivp_osc(
                omega_fun, gamma_fun, (left_boundary, midpoint), [yi, dyi],
            ...
            init_step = _ric.choose_osc_stepsize(
                omega_fun, gamma_fun, right_boundary, right_boundary - midpoint, epsh,
                **solver_params
            )
            right_res = solve_ivp_osc(
                omega_fun, gamma_fun, (right_boundary, midpoint), [yi, dyi],
    ...
    def test_bremer_nondense(self):
        ...
        for lambda_scalar in lambdas:
            ...
            init_step = _ric.choose_nonosc_stepsize(
                w, g, xi, 1.0, epsh, **solver_params
            )
            res = solve_ivp_osc(
                w, g, (xi, xf), [yi, dyi],
                rtol=eps, epsilon_h=epsh, init_stepsize=init_step,
                events=None, vectorized=False, **solver_kwargs
            )
    ...
    def test_solve_burst(self):
        ...
        res = solve_ivp_osc(
            w, g, (xi, xf), [yi, dyi],
            rtol=eps, epsilon_h=epsh, init_stepsize=0.01,
            events=None, vectorized=False
        )
        xs = res.t
        ys = res.y
...
class TestVectorizedFalse:
    """Test scalar-only functions with vectorized=False using Vectorizer."""

    def test_scalar_only_cosine(self):
        """Test that scalar-only functions work with vectorized=False."""
        # These functions use float() which would fail on arrays
        omega = lambda x: float(1.0)
        gamma = lambda x: float(0.0)

        t_span = (0, 2*np.pi)
        y0 = [1.0, 0.0]
        result = solve_ivp_osc(omega, gamma, t_span, y0, rtol=1e-8,
                               vectorized=False)
        assert result.success
        y_true = np.cos(result.t)
        error = np.abs(result.y - y_true) / (1 + np.abs(y_true))
    ...
    def test_scalar_only_airy(self):
        """Test scalar-only Airy equation with vectorized=False."""
        w = lambda x: float(np.sqrt(x))
        g = lambda x: 0.0
        xi, xf = 1.0, 50.0
        eps = 1e-8
        epsh = 1e-9
        ...
        res = solve_ivp_osc(w, g, (xi, xf), [yi, dyi],
                            rtol=eps, epsilon_h=epsh, init_stepsize=0.05,
                            vectorized=False)
        ytrue = np.array([special.airy(-x)[0] + 1j * special.airy(-x)[2]
                          for x in res.t])
        yerr = np.abs((ytrue - res.y) / ytrue)
        assert np.max(yerr) < 1e-4

    def test_scalar_float32_return(self):
        """Test that numpy float32-returning functions work with vectorized=False."""
        omega = lambda x: np.float32(1.0)
        gamma = lambda x: np.float32(0.0)

        result = solve_ivp_osc(omega, gamma, (0, 2*np.pi), [1.0, 0.0],
                               rtol=1e-8, vectorized=False)
        assert result.success
        y_true = np.cos(result.t)
        error = np.abs(result.y - y_true) / (1 + np.abs(y_true))
        assert np.max(error) < 1e-5

    def test_scalar_complex64_return(self):
        """Test that numpy complex64-returning omega works with vectorized=False."""
        omega = lambda x: np.complex64(1.0 + 0j)
        gamma = lambda x: 0.0
