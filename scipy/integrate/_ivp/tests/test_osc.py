...
class TestSolveIvpOsc:
    ...
    def test_validation_atol_not_numeric(self):
        """Test that atol must be numeric."""
        with pytest.raises(TypeError, match="`atol` must be a positive float"):
            solve_ivp_osc(lambda x: 1, lambda x: 0, (0, 1), [1.0, 0.0], atol="bad")

    def test_validation_atol_not_positive(self):
        """Test that atol must be positive."""
        with pytest.raises(ValueError, match="`atol` must be positive"):
            solve_ivp_osc(lambda x: 1, lambda x: 0, (0, 1), [1.0, 0.0], atol=0)
    ...
    def test_validation_events_not_implemented(self):
        ...
        with pytest.raises(NotImplementedError, match="events are not yet supported"):
            solve_ivp_osc(lambda x: 1, lambda x: 0, (0, 1), [1.0, 0.0], events=event)

    def test_validation_args_not_implemented(self):
        """Test that args are not yet supported."""
        with pytest.raises(NotImplementedError, match="args are not yet supported"):
            solve_ivp_osc(lambda x: 1, lambda x: 0, (0, 1), [1.0, 0.0], args=(1,))
